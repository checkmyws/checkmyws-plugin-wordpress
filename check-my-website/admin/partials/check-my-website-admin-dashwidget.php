...
if ( $data['global'] ) {
	...
	echo '<div class="cmws-column cmws-column-6"><strong>' . __( 'Last time response:', 'check-my-website' ) . '</strong> ' . convert( $data['global']['last_time_response'], $default_unit ) . '<span class="cmws-widget-font"> ' . $default_unit . '</span></div>';
	echo '<div class="cmws-column cmws-column-6"><strong>' . __( 'Average time (24h):', 'check-my-website' ) . '</strong> ' . convert( $data['global']['average_time'], $default_unit ) . '<span class="cmws-widget-font"> ' . $default_unit . '<span></div>';
} else {
...
if ( $data['yslow'] ) {
	...
	echo '<div class="cmws-column cmws-column-6"><strong>' . __( 'Score:', 'check-my-website' ) . '</strong> <span class="cmws-font">' . $data['yslow']['score'] . '</span></div><div class="cmws-column cmws-column-6"><strong>' . __( 'Page load time:', 'check-my-website' ) . '</strong> <span class="cmws-font">' . convert( $data['yslow']['page_load_time'], 's' ) . 's' . '</span></div>';
	echo '</p>';
} else {
	echo '<p><span class="cmws-danger">' . __( 'No data', 'check-my-website' ) . '</span></p>';
}
